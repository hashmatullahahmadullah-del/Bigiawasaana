...
initializeApp({ projectId: 'bigi-awasaana-7b3ce' });
const db = getFirestore();

async function run() {
  let count = 0;
  for (const area of areas) {
    const docRef = db.collection('serviceAreas').doc(area.id);
    
    // We are generating SEO optimized content for each neighborhood!
    const data = {
      id: area.id,
      name: area.name,
      title: `Halal Afghan Food Delivery in ${area.name} | Bigi Awasaana`,
      metaDescription: `Looking for authentic Halal Afghan food in ${area.name}? Bigi Awasaana delivers fresh kabobs, bolani, and Afghan bakery items directly to you. Order now!`,
      content: `
  ...
  console.log(`\n🎉 Successfully pushed ${count} service areas to Firestore!`);
}
